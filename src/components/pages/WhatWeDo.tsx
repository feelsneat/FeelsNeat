    ...
    <main className="flex-1 w-full bg-[#FAFAFA] py-20 sm:py-28 border-b border-zinc-200/50 relative">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 relative z-10">
        ...
        <div className="max-w-3xl mb-20 scroll-reveal">
          <span className="text-[10px] font-black text-[#6E8270] uppercase tracking-widest block mb-3">Capabilities</span>
          ...
          {services.map((service, index) => (
            ...
            >
              ...
              <div className="md:col-span-4 flex items-center md:items-start gap-4">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-zinc-100 border border-zinc-200 text-[#6E8270] shadow-xs">
                  <LucideIcon name={service.icon} className="h-5 w-5" />
                </div>
                <div>
                  <h2 className="text-lg font-bold text-[#1E1E1E] uppercase tracking-tight leading-tight">{service.title}</h2>
                  <span className="text-[9px] text-[#6E8270] uppercase tracking-widest font-black block mt-1.5">Capability 0{index + 1}</span>
                </div>
              </div>

              {/* Service description */}
              <div className="md:col-span-8 space-y-8">
                <p className="text-base font-bold text-[#1E1E1E] leading-relaxed uppercase tracking-tight">
                  {service.summary}
                </p>

                {/* Main Service Image Graphic */}
                {service.image && (
                  <div className="w-full aspect-[2.1/1] rounded-lg overflow-hidden border border-zinc-200 bg-zinc-100/50 shadow-xs">
                    <img 
                      src={service.image} 
                      alt={service.title} 
                ...
                <div 
                  className="text-sm text-[#5F5F5F] space-y-6 leading-relaxed prose max-w-none font-medium"
                ...
                {service.additionalImages && service.additionalImages.length > 0 && (
                  <div className="pt-4 space-y-4">
                    <h4 className="text-[9px] font-black text-[#5F5F5F] uppercase tracking-widest">Capability Gallery</h4>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                      {service.additionalImages.map((imgUrl, imgIdx) => (
                        <div key={imgIdx} className="aspect-[1.5/1] rounded-lg overflow-hidden border border-zinc-200 bg-zinc-50 shadow-xs">
                          <img 
                            src={imgUrl} 
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-6 border-t border-zinc-200">
                  <div className="flex flex-wrap gap-1">
                    {(service.tags || []).map((tag) => (
                      <span key={tag} className="inline-flex items-center rounded-md bg-zinc-100 border border-zinc-200 px-2.5 py-0.5 text-[9px] font-black uppercase text-[#1E1E1E]/70">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  {service.externalUrl && (
                    ...
                    >
                      Explore resource <LucideIcon name="ArrowUpRight" className="h-3.5 w-3.5 text-[#6E8270]" />
                    </a>
                  )}
                </div>
              </div>
